    ...
    .map(it => {
      ...
      return (
        <div class="pin-chip" data-key="${it.__key}">
          ...
          ${
            ars
              ? <span class="pin-price" style="white-space: nowrap;">
                  ${ars}${usd}
                </span>
              : (
                usd
...
function ensureAnchorMenu () {
  ...
  panel.innerHTML =
    <button
      data-action="copy"
      style="width:100%;text-align:left;background:none;border:0;color:var(--text);padding:10px 12px;border-radius:10px;"
    >
      📋 Copiar cubierta
    </button>
    <button
  ...
  data.forEach(item => {
    ...
    const priceHtml =
      (precioUsd != null)
        ? <div
            class="price-wrap"
            style="display:flex;flex-direction:column;gap:2px;align-items:flex-end;"
          >
            ${
              precioArs != null
                ? <span class="price-ars" style="font-weight:600;">
                    ${fmtARS(precioArs)}
                  </span>
                : ''
            }
            ${
              soloPesos
                ? ''
                : <span class="price-usd" style="opacity:.8;">
                    ${fmtUSD(precioUsd)}
                  </span>
            }
          </div>
    ...
    const copyText = [
      codigoDisplay,
      item.descripcion || '',
      (precioArs != null ? fmtARS(precioArs) : fmtUSD(precioUsd))
    ]
      .filter(Boolean)
      .join(' ')
      .trim()
    ...
    const anchorBtnHTML =
      <button
        class="anchor-btn ${pinned.has(key) ? 'active' : ''}"
...
function ensureUsdInline () {
  ...
  line.innerHTML =
    ...
    <span class="muted">
      (<span id="usd-inline-label">Oficial</span>)
    </span>
    <span class="sep">—</span>
